...
import { ArrowRight, BarChart3, Users, TrendingUp, Sparkles, UserCircle } from 'lucide-react';
...
export function LandingPage({ onShowLogin, onShowProfile, onShowInterest, onShowStats }: LandingPageProps) {
  const [showLogin, setShowLogin] = useState(false);
  const [showSignup, setShowSignup] = useState(false);
  const [loginTrigger, setLoginTrigger] = useState<'button' | 'nav' | null>(null);
        ...
        >
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => handleNavClick(onShowStats)}
                className="text-white/80 hover:text-white transition-colors px-4 py-2"
              >
                Data Insights
              </button>
              <button
                onClick={() => handleNavClick(onShowProfile)}
                className="flex items-center gap-2 px-4 py-2 border border-white/30 rounded-lg text-white hover:bg-white/10 transition-colors"
              >
              ...
              <button
                ...
                className="px-6 py-2.5 bg-white text-slate-900 rounded-lg font-semibold hover:bg-white/90 transition-all"
              >
                Login
              </button>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center relative z-10">
            ...
            >
              ...
              <p className="text-xl text-white/70 leading-relaxed max-w-xl">
                브랜드 가치에 공감하는 인플루언서를 탐색하세요. 데이터 기반 분석부터 맞춤형 검색까지 해결할 수 있습니다.
              </p>

              <div className="flex items-center gap-4">
                <button
                  onClick={handleLoginClick}
                  className="group px-8 py-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold text-lg hover:shadow-lg hover:shadow-purple-500/50 transition-all flex items-center gap-2"
                >
                  Get Started
              ...
              {[
                {
                  icon: Users,
                  title: '편리한 탐색',
                  description: '원하는 인플루언서를 찾아보세요. 원하는 필터를 적용하여 자유롭게 검색할 수 있습니다.',
                },
                {
                  icon: BarChart3,
                  title: '자유로운 커스터마이징',
                  description: '원하는 인플루언서를 기록하세요. 관심 목록에 저장하고 메모할 수 있습니다.',
                },
                {
                  icon: TrendingUp,
                  title: '자세한 분석',
                  description: '원하는 인플루언서를 분석하세요. 인기 있는 인플루언서 순위를 확인하고 비교할 수 있습니다.',
                },
              ].map((feature, index) => (
                <motion.div
                  key={index}
                  ...
                  className="group p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 transition-all"
                >
        ...
        <div className="relative py-32">
          <div className="max-w-4xl mx-auto px-6 text-center">
            ...
            >
              ...
              <button
                onClick={handleLoginClick}
                className="px-10 py-5 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold text-lg hover:shadow-lg hover:shadow-purple-500/50 transition-all inline-flex items-center gap-3"
              >
                Start Your Journey
