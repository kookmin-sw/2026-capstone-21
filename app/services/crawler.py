...
class CrawlerService:
    ...
    def _save_to_db(self, final_df):
        ...
        for _, row in final_df.iterrows():
            ...
            metrics = calculate_upload_metrics_dict(
                inf_item.get("latestPosts", [])
            )
            ...
            if "latestPosts" in inf_item and inf_item["latestPosts"]:
                create_influencer_posts(
                    self.db,
                    db_influencer.influencer_id,
                    inf_item["latestPosts"],
                )

            if "relatedProfiles" in inf_item and inf_item["relatedProfiles"]:
                related_unames = [
                    r.get("username")
                    for r in inf_item["relatedProfiles"]
                    if r.get("username")
                ]

                create_related_relations(
                    self.db,
                    db_influencer.influencer_id,
                    related_unames,
                )
    ...
    def run_targeted_crawl(
        self,
        base_seeds: list[str] | None = None,
        ...
        last_post_date = filters.get("last_post_date") or "2025-09-01"

        if base_seeds:
            final_df = expand_seed(
                existing_usernames=existing_usernames,
                seed_brand=base_seeds,
                max_limit=max_limit,
                only_posts_newer_than=last_post_date,
            )
        else:
...
def filter_influencer(
    row,
    min_f=MIN_FOLLOWERS,
    min_p=MIN_POSTS,
    f_ratio=FOLLOW_RATIO,
    e_rate=ENGAGEMENT_RATE,
):
    ...
    avg_en_rate = avg_engagement_per_post / max(1, followers)
...
def preprocess_df(
    ...
    for col in ["followersCount", "followsCount", "postsCount"]:
        ...
        df_unique[col] = pd.to_numeric(
            df_unique[col],
            errors="coerce",
        ).fillna(0)

    influencer_df = df_unique[
        df_unique.apply(
            filter_influencer,
            axis=1,
            args=(min_f, min_p, f_ratio, e_rate),
        )
    ].copy()
...
def expand_seed(
    ...
    client=client,
    max_limit=50,
    only_posts_newer_than="2025-09-01",
):
    seed_targets = []
    ...
    for seed in seed_brand:
        ...
        if "ownerUsername" in hashtag_df.columns:
            seed_targets.extend(
                hashtag_df["ownerUsername"].dropna().unique().tolist()
            )

    new_targets = [
        username
        for username in set(seed_targets)
...
def calculate_upload_metrics_dict(latest_posts: list):
    ...
    intervals = [
        (dates[i] - dates[i - 1]).total_seconds() / (24 * 3600)
    ...
    return {
        ...
        "posts_per_week": round(7 / avg_interval, 1)
        if avg_interval > 0
        else 0,
    }
