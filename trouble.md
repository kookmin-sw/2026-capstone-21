...
## AH. 챗봇 한글 입력 시 Enter로 메시지 중복 전송
- **증상**: 한글 입력 후 Enter 누르면 조합 중인 글자가 확정되면서 메시지가 의도치 않게 전송됨
- **원인**: IME 조합(composition) 중에도 `keydown` 이벤트의 Enter가 `handleSend()`를 트리거
- **해결**: `useRef`로 `isComposing` 상태 관리, `onCompositionStart`/`onCompositionEnd` 이벤트로 조합 상태 추적, 조합 중이면 Enter 무시

## AI. 챗봇 답변 수신 시 불필요한 자동 스크롤
- **증상**: 이전 대화를 위로 스크롤해서 읽는 중에 봇 응답이 오면 강제로 맨 아래로 이동
- **원인**: `messages` 상태 변경 시 무조건 `scrollIntoView` 호출
- **해결**: `messagesContainerRef`의 `onScroll`로 사용자가 하단에 있는지 감지(`scrollHeight - scrollTop - clientHeight < 50`), 하단에 있을 때만 자동 스크롤 수행

## AJ. 챗봇 질문 유형 분류 오류 (키워드 충돌)
- **증상**: "인플루언서 추천 기능 어떻게 사용하나요?" 같은 질문이 추천으로 분류되어 엉뚱한 응답
- **원인**: "추천" 키워드가 사이트 이용/인플루언서 추천 양쪽에 모두 매칭되는데, 우선순위만으로 처리
- **해결**: 양쪽 키워드가 동시에 매칭되면 GPT(`gpt-4o-mini`)로 의도 판별하는 `_classify_intent_with_gpt` 메서드 추가. 상품/브랜드 설명 패턴(분위기, 스타일, 용품 등)도 추천으로 인식하도록 확장

## AK. 추천 결과 0개 시 빈 화면
- **증상**: 특정 쿼리에서 임계값(0.55) 통과하는 인플루언서가 없어 결과 0개
- **원인**: 임계값이 고정이라 데이터 분포에 따라 전부 컷될 수 있음
- **해결**: 결과 0개일 때 임계값을 0.3으로 완화하여 재시도, 최대 20개까지 반환하는 폴백 로직 추가

## AL. 회원가입 시 쇼핑몰 정보 미저장
